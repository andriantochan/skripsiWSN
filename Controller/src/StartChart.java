...
public class StartChart extends Application{
	...
	public void addData(String[] hasil) {
		if(hasil[0].charAt(0)=='2') {
			...
			dataFrequency.setX(Double.parseDouble(df.format(t1)));
			dataFrequency.setY(Double.parseDouble(df.format(t2)));
			dataFrequency.setZ(Double.parseDouble(df.format(t3)));
...
//		stage2.setScene(scene);
		...
		scheduledExecutorService.scheduleAtFixedRate(() -> {
			...
			Platform.runLater(() -> {
				...
				//taro random number dengan waktu skrng
				if(tempX != dataFrequency.X) {
					toggle = true;
				}
				else if(tempY != dataFrequency.Y) {
					toggle = true;
					
				}
				else if(tempZ != dataFrequency.Z) {
					toggle = true;
				}
				
				if(toggle) {
					series1.getData().add(new XYChart.Data<>(simpleDateFormat.format(now), dataFrequency.X));
					series2.getData().add(new XYChart.Data<>(simpleDateFormat.format(now), dataFrequency.Y));
					series3.getData().add(new XYChart.Data<>(simpleDateFormat.format(now), dataFrequency.Z));
